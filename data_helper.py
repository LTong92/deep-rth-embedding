...
class DataHelper(object):
    ...
    def _gen_triples(self, batch_size):
        ...
        index = np.random.choice(self._train_frames[pos_label].shape[0], batch_size+1, replace=False)
        pos_sample0 = self._train_frames[pos_label][index[0]]
        pos_sample0 = np.expand_dims(pos_sample0, axis=0)
        pos_sample0 = np.repeat(pos_sample0, batch_size, axis=0)
